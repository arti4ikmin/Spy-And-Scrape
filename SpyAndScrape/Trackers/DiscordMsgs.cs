...
public class DiscordMsgs
{
    ...
    public async Task StartTrackingAsync()
    {
        ...
        while (!_cancellationTokenSource.IsCancellationRequested)
        {
            try
            {
                ...
                Console.WriteLine($"[DiscordMsgs] Tryin to connect to Discord Gateway: {targetGatewayUrl}");
            ...
            {
                Console.WriteLine($"[DiscordMsgs] Err during Discord tracking: {ex.Message} Attempting to reconnect...");
            }
            finally
            {
    ...
    private async Task HandleGatewayMessageAsync(string jsonMessage, CancellationToken token)
    {
        ...
        {
            Console.WriteLine($"[DiscordMsgs] Err deserializing Gateway message: {ex.Message} Payload: {jsonMessage}");
        }
        catch (Exception ex)
        {
             Console.WriteLine($"[DiscordMsgs] Err handling Gateway message: {ex.ToString()} Payload: {jsonMessage}");
        }
    }
    ...
    private async Task HeartbeatLoopAsync(CancellationToken token)
    {
        ...
        // first hb after interval * jitter (rand fraction between 0 and 1)
        ...
        while (!token.IsCancellationRequested && _gatewaySocket.State == WebSocketState.Open)
        {
            ...
            {
                Console.WriteLine("[DiscordMsgs] Hb loop cancelled");
                break;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[DiscordMsgs] Err in hb loop: {ex.Message}");
    ...
    private async Task SendJsonAsync(object payload, CancellationToken token)
    {
        ...
        {
            Console.WriteLine("[DiscordMsgs] Cannot send JSON, WS is not open");
            return;
        }
